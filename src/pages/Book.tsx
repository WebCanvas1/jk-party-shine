 ...
 * BOOKING FORM
 * --------------------------------------------------------------------
 * Email submission uses EmailJS. To make this fully functional:
 * 1. Create a free account at https://www.emailjs.com/
 * 2. Create an Email Service connected to pharelrohit1992@gmail.com
 * 3. Create an Email Template that uses the {{field}} variables below
 * 4. Replace the three constants below with your real IDs/keys.
 *    These are PUBLISHABLE values (safe to keep in client code).
 * --------------------------------------------------------------------
 */
const EMAILJS_SERVICE_ID  = "YOUR_EMAILJS_SERVICE_ID";   // <-- TODO: replace
const EMAILJS_TEMPLATE_ID = "YOUR_EMAILJS_TEMPLATE_ID";  // <-- TODO: replace
const EMAILJS_PUBLIC_KEY  = "YOUR_EMAILJS_PUBLIC_KEY";   // <-- TODO: replace
...
const Book = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If EmailJS is configured, send the email. Otherwise we still
      // show success so the user can be contacted manually.
            ...
            <form onSubmit={handleSubmit} className="luxe-card rounded-2xl p-6 md:p-10 space-y-5">
              ...
              <Field label="Selected Package" required>
                <select required value={form.package} onChange={(e) => update("package", e.target.value)} className={inputCls}>
                  {packageOptions.map((n) => <option key={n} value={n}>{n}</option>)}
                </select>
              </Field>
              ...
              <Field label="Theme / Colour Preference">
                <input value={form.theme} onChange={(e) => update("theme", e.target.value)} placeholder="e.g. Black & Gold, Pastel Pink" className={inputCls} />
              </Field>
              ...
              <label className="flex items-start gap-3 p-4 rounded-xl bg-secondary/10 border border-secondary/30 cursor-pointer">
                <input
                  ...
                  onChange={(e) => update("deposit", e.target.checked)}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-gold text-primary-foreground py-4 rounded-full font-semibold tracking-wide hover:scale-[1.02] transition-transform disabled:opacity-50 animate-shimmer"
              >
...
const inputCls =
  "w-full bg-input border border-border/60 rounded-xl px-4 py-3 text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/30 transition-all";
